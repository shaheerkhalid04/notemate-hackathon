...
# Make sure collection_name always exists
...
if 'generator' not in st.session_state:
    ...
    # Fallback: environment variable (for local .env)
    if not api_key:
...
if st.session_state.collection_name:
    ...
    with tabs[2]:
        ...
        concept = st.text_input(
            "Enter concept to storify",
            placeholder="e.g., Database Indexing, Cell Division, Supply and Demand"
        )
    ...
    with tabs[4]:
        ...
        with col1:
            chapters = st.text_area(
                "Enter chapters/topics (one per line)", 
    ...
    # Welcome screen
    st.info("👈 Please upload a document to start generating AI-powered learning materials")
